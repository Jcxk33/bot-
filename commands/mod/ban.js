...
module.exports = class uban extends Command {
    ...
    super(client, {
      ...
      args: [
        ...
        },
        {
          type: "string",
          prompt:
            "Insert the time for this ban. **ex (30s, 20m, 1h, 1w, 1y, forever)**",
          key: "time"
        },
        {
          type: "string",
  ...
  async run(msgObject, { argUser, time, reason }) {
    ...
    if (argUser.roles.cache.some(role => role.name === "Moderator") || argUser.roles.cache.some(role => role.name === "Admin") || argUser.roles.cache.some(role => role.name === "Trial Moderator") || argUser.roles.cache.some(role => role.name === "Head Moderator") || argUser.roles.cache.some(role => role.name === "Senior Admin")) {
        ...
        */
      if (
        msgObject.member.role.cache.some(role => role.name === "Bot Developer")
      ) {
        /// if person running cmd is moderator
        if (
          argUser.roles.cache.some(role => role.name === "Moderator") ||
          argUser.roles.cache.some(role => role.name === "Trial Moderator") ||
          argUser.roles.cache.some(role => role.name === "Admin") ||
          argUser.roles.cache.some(role => role.name === "Head Moderator") ||
          argUser.roles.cache.some(role => role.name === "Senior Admin")
        ) {
          /// checks if the arguser is admin, head mod, or senior admin
          return msgObject.reply(
            "Sorry, you cannot ban this person. This guy is a higher rank than you, or is a trial moderator."
          );
        }
        ...
        if (
          argUser.roles.cache.some(role => role.name === "Head Moderator") ||
          argUser.roles.cache.some(role => role.name === "Bot Developer")
        ) {
          /// checks if argUser is a head mod or senior admin
          return msgObject.reply(
            "Sorry, you cannot ban this person. This guy is a higher rank than you."
          );
        }
      } else if (
        msgObject.member.roles.cache.some(
          role => role.name === "Head Moderator"
        )
      ) {
        //// checks if head mod
    ...
    } else if (immuneIDs.includes(argUser.id)) {
      if (argUser.id === msgObject.author.id) {
        const msg = await msgObject.reply(
          "Hey, you seem to be immune. Would you like to proceed? ||idk why would you but ok||"
        );
        msg.react("👍").then(m => {
          msg.react("👎");
      ...
      this.client.guilds.cache.each(m => {
        if (newTime === "forever") {
          ...
          setTimeout(function() {
            msgObject.channel.guild.members.unban(argUser.id);
            msgObject.member.send(
              "Just a heads-up, <@" + argUser.id + "> has been unbanned,"
            );
          }, ms(time));
        }
      });
      ...
      msg.react("💣").then(m => {
        ...
        const filter = (reaction, user) => {
          ...
          if (
            ...
            msgObject.reply("Coolio :joy::joy:! Let's ban em'! :gun:");
            this.client.guilds.cache.each(m => {
              if (newTime === "forever") {
                /// edit "forever"
                let options = { reason: reason };
